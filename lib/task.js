...
Task.prototype.ap = function _ap(that) {
  ...
  return new Task(function(reject, resolve) {
    var doneCount = 0;
    var allState;
    var thisState = forkThis(guardFn(reject, true), guardFn(resolve));
    var thatState = forkThat(guardVal(reject, true), guardVal(resolve));

    return allState = [thisState, thatState];

    function guardFn(f, isFail) {
      return function(x) {
        if(isFail) return f(x);
        doneCount++;
